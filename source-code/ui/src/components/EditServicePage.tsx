...
type EditServicePageProps = {
  ...
  onLogout: () => void;
  onEditService: (serviceId: string, updatedData: Partial<ServiceListing>) => void;
};

export function EditServicePage({ user, service, onNavigate, onLogout, onEditService }: EditServicePageProps) {
  const [title, setTitle] = useState(service.title);
  const [description, setDescription] = useState(service.description);
  const [categories, setCategories] = useState<string[]>(service.categories);
  const [hashtags, setHashtags] = useState<string[]>(service.hashtags || []);
  const [hashtagInput, setHashtagInput] = useState('');
  const [price, setPrice] = useState(service.price.toString());
  const [priceType, setPriceType] = useState<'tetap' | 'per-jam' | 'mulai-dari'>(service.priceType);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header user={user} onNavigate={onNavigate} onLogout={onLogout} currentPage="edit-service" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Jasa</h1>
          <p className="text-gray-600 mb-6">Perbarui informasi jasa Anda</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {suggestedHashtags.length > 0 && hashtags.length < 8 && (
                <div>
                  <p className="text-xs text-gray-500 mb-2">Saran tagar:</p>
                  <div className="flex flex-wrap gap-2">
